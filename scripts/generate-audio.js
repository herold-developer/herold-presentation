...
async function main() {
  ...
  try {
    ...
    const concatData = scriptParts.map((part) => `file '${part.filename}'`).join('\n')
    fs.writeFileSync(concatFile, concatData)

    try {
      execSync(
        `cd "${audioDir}" && ffmpeg -y -f concat -safe 0 -i concat.txt -c copy herold-presentation.mp3`,
        { stdio: 'ignore' }
      )
    } catch (error) {
